<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="GraphData">
		...
		<xsl:apply-templates select="originalEquationPrint" />
		
		<xsl:apply-templates select="graphName" />

		<xsl:choose>
			<xsl:when
				test="graphName='line' or graphName='horizontal line' or graphName='vertical line'">
		...
		<xsl:apply-templates select="yIntercepts">
			<xsl:with-param name="axis">
				<xsl:value-of select="ordinateSymbol" />
			</xsl:with-param>
		</xsl:apply-templates>
	...
	<xsl:template name="polarLemniscateSpecifics">
		The graph looks like a figure eight or the mathematical symbol for
		infinity.
	</xsl:template>
	...
	<xsl:template name="nestedLoopsSpecifics">
		...
		c small compared to a and b, the smaller loops come close to drawing
		over the larger ones, and you approach
